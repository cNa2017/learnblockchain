...
/**
 * @title SimpleV2Pair
 * @dev 简化版Uniswap V2交易对合约，支持TWAP价格预言机
 */
contract SimpleV2Pair is SimpleV2ERC20 {
    ...
    // TWAP价格累积变量 - 使用简化的整数累积方法
    uint256 public price0CumulativeLast; // token0 相对于 token1 的累积价格
    uint256 public price1CumulativeLast; // token1 相对于 token0 的累积价格
    ...
    function getCurrentCumulativePrices()
        ...
        // 如果当前区块时间戳大于最后更新时间，需要加上时间差内的价格累积
        if (blockTimestamp > blockTimestampLast) {
            uint32 timeElapsed = blockTimestamp - blockTimestampLast;
            if (reserve0 != 0 && reserve1 != 0) {
                // price0 = reserve1 / reserve0 * PRICE_PRECISION
                // price1 = reserve0 / reserve1 * PRICE_PRECISION
                price0Cumulative += (uint256(reserve1) * PRICE_PRECISION / reserve0) * timeElapsed;
                price1Cumulative += (uint256(reserve0) * PRICE_PRECISION / reserve1) * timeElapsed;
            }
        }
    }
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1)
        ...
        // 如果时间已过去且储备量都不为零，则更新价格累积
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            // 累积价格 = 之前的累积 + (当前价格 * 时间间隔)
            // price0 = reserve1 / reserve0，表示1个token0能换多少token1
            // price1 = reserve0 / reserve1，表示1个token1能换多少token0
            price0CumulativeLast += (uint256(_reserve1) * PRICE_PRECISION / _reserve0) * timeElapsed;
            price1CumulativeLast += (uint256(_reserve0) * PRICE_PRECISION / _reserve1) * timeElapsed;
        }
